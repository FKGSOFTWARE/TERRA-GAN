#!/bin/bash
#
# run_experiment_ablation.sh - Orchestrate the complete ML pipeline experiment (Ablation Study)
#
# This script automates the entire process without human-guided training:
# 1. Clean environment
# 2. Run training on all grids sequentially
# 3. Run evaluation and upload results for each grid
...
LOG_FILE="$LOGS_DIR/experiment_ablation_${TIMESTAMP}.log"
...
read -p "Enter experiment name (ablation study): " EXPERIMENT_NAME
if [ -z "$EXPERIMENT_NAME" ]; then
    EXPERIMENT_NAME="ml_ablation_${TIMESTAMP}"
...
# Steps 4-5: Evaluation and upload results for each grid (no human guided training)
log "Starting evaluation phase (ablation study - no human guided training)..."

for i in "${!TRAINING_ZIPS[@]}"; do
    ZIP="${TRAINING_ZIPS[$i]}"
    GRID_NAME=$(basename "$ZIP" .zip)

    log "Processing grid $((i+1))/${#TRAINING_ZIPS[@]} for evaluation: $GRID_NAME"
    ...
    # Note in log that we're skipping human-guided training
    log "ABLATION STUDY: Skipping human-guided training for $GRID_NAME"

    # Clean server resources that would normally be handled after human-guided training
    clean_server_annotations "$GRID_NAME"
    log "Cleaned server annotations for $GRID_NAME"
done

log "Evaluation phase completed for all grids (ablation study - no human guided training)"

# Step 6-7: Final evaluation on separate test grid
...
log "Ablation experiment completed!"
...
echo "=================================================================="
echo "ABLATION EXPERIMENT COMPLETED: $EXPERIMENT_NAME - $OVERALL_SUCCESS"
echo "This experiment omitted human-guided training"
